export function layout(title, content) {
  ...
  export function list(posts) {
    ...
    for (let post of posts) {
      ...
      <li>
        <h2>${post.title}</h2>
        <p><a class="post-link" href="/post/${post.id}">閱讀全文</a></p>
    ...
    let content = `
      ...
      <p><a href="/post/new" class="post-link">撰寫新文章</a></p>
      <ul id="posts">
        ${list.join('\n')}
      </ul>
    `
    return layout('部落格文章列表', content)
  }
  ...
  export function show(post) {
    ...
    return layout(post.title, `
      ...
      <div>${post.body}</div>
      <p class="post-time">發布時間：${formattedDate}</p>
      <p><a href="/" class="post-link">返回文章列表</a></p>
    `);
  }
